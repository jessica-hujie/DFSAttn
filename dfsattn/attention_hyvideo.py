...
class DFS_Attention(nn.Module):
    ...
    def _compute_block_mask(self, q: torch.Tensor, k: torch.Tensor, permutations: torch.Tensor, sparse_ratio: Optional[float] = None) -> torch.Tensor:
        ...
        # Include text key blocks for each query block
        if num_text_blocks > 0:
            video_block_sums = block_value_sums[:, :, :, :dense_start_block]
                
            # Calculate how many video blocks to select
            ...
            # Select top-k video blocks
            _, video_top_indices = torch.topk(video_block_sums, k=num_video_blocks_to_select, dim=-1)  # (1, num_heads, video_blocks_num, num_video_blocks_to_select)
                    
            # Create text block indices (dense_start_block to k_blocks-1)
            text_indices = torch.arange(dense_start_block, k_block_num, device=q.device, dtype=torch.long)
            text_indices = text_indices.unsqueeze(0).unsqueeze(0).unsqueeze(0)  # (1, 1, 1, num_text_blocks)
            text_indices = text_indices.expand(1, num_heads, video_blocks_num, -1)  # (1, num_heads, video_blocks_num, num_text_blocks)
            topk_indices = torch.cat([video_top_indices, text_indices], dim=-1)  # (1, num_heads, video_blocks_num, top_k_blocks)
        ...
        # video_mask now includes k_blocks (video + text) to handle top_indices that may include text blocks
        video_mask = torch.zeros((1, num_heads, video_blocks_num, k_block_num), dtype=torch.bool, device=q.device)
        # Scatter True values for each q_block at the selected k_blocks positions
